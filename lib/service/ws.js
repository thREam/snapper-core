...
const redisClient = require('./redis').defaultClient
const RPCCommand = require('toa-net').RPCCommand
...
class WSCommand extends RPCCommand {
  _encode (method, params) {
    return JSON.stringify(jsonrpc.request(this.id, method, params))
  }
}
...
engine.Socket.prototype.sendMessages = function (messages) {
  return thunk.call(this, (callback) => {
    this.pendingRPC = new WSCommand('publish', messages, callback)
    this.pendingRPC.cleanup.push(() => {
